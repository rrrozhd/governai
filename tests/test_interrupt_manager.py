...
@pytest.mark.asyncio
async def test_interrupt_create_list_resolve() -> None:
...
@pytest.mark.asyncio
async def test_interrupt_epoch_guard() -> None:
    mgr = InterruptManager(default_ttl_seconds=60)
...
@pytest.mark.asyncio
async def test_interrupt_expiration_cleanup() -> None:
    mgr = InterruptManager(default_ttl_seconds=60)
...
@pytest.mark.asyncio
async def test_interrupt_max_pending_enforced() -> None:
    mgr = InterruptManager(default_ttl_seconds=60)
...
@pytest.mark.asyncio
async def test_redis_interrupt_store_persists_requests_and_epochs() -> None:
    fake = AsyncFakeRedis()
...
@pytest.mark.asyncio
async def test_redis_interrupt_store_preserves_expired_and_epoch_mismatch_behavior() -> None:
    fake = AsyncFakeRedis()
    mgr_a = InterruptManager(
        default_ttl_seconds=60,
        store=RedisInterruptStore(redis_url="redis://unused", redis_client=fake),
